...
void Game::move()
{
	if (Direction::None != m_direction)
	{
		...
		if (Direction::left == m_direction && m_target.x > m_location.x)
		{
		...
		if (Direction::Right == m_direction && m_target.x < m_location.x)
		{
			m_direction = Direction::None;
			m_frameIncrement = 0.25f;
			m_sound.setPitch(0.5f);
...
void Game::setupFontAndText()
{
	...
	m_welcomeMessage.setString("Helicopter Game");
...
void Game::setupSprite()
{
	...
	m_heloSprite.setTextureRect(sf::IntRect{ 0, 0, 184, 64 });
	m_heloSprite.setPosition(m_location);
